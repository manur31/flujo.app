...
function ProductForm({closeModal, createProduct}) {
  ...
  return (
    <main className='flex flex-col items-center justify-items-center gap-10 bg-[#1E293B] w-full h-fit text-white py-10 px-4'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-4 gap-y-8'>
            <article className='flex flex-col items-center gap-2 w-full'>
                ...
                <input onChange={(e)=> setName(e.target.value)} type="text" name="name" placeholder='Paquete de Jamon...' className='border-2 rounded-xl px-4 py-2 text-neutral-200'/>
            </article>
            <article className='flex flex-col gap-2 w-full'>
                <label htmlFor="price" className='text-2xl px-2'>Precio <span className='text-red-400'>*</span></label>
                <input onChange={(e)=> setPrice(e.target.value)} type="number" name="price" placeholder='250..' className='border-2 rounded-xl px-4 py-2 text-neutral-200'/>
            </article>
